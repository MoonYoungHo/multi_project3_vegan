...
def C1_get_datasets():
    # 첫번째 데이터셋
    file_path = '../../2.데이터셋(프로젝트_클러스터링)/bbcgoodfood_all.json'
    with open(file_path, 'r', encoding='utf-8-sig') as file:
        raw_data = json.load(file)

    df1=pd.DataFrame(raw_data["bbcgoodfood"])
    df1=df1[['title','ingredients','time','serving','recipe','nutrition','image']]


    # 두번째 데이터셋
    file_path = '../../2.데이터셋(프로젝트_클러스터링)/feastingathome_all.json'
    with open(file_path, 'r', encoding='utf-8-sig') as file:
        raw_data = json.load(file)

    df2=pd.DataFrame(raw_data["feastingathome"])
    df2=df2[['title','ingredients','time','serving','recipe','nutrition','image']]


    # 세번째 데이터셋
    file_path = '../../2.데이터셋(프로젝트_클러스터링)/lazycatkitchen_review_all.json'
    with open(file_path, 'r', encoding='utf-8-sig') as file:
        raw_data = json.load(file)

    df3=pd.DataFrame(raw_data["lazycatkitchen"])
    df3=df3[['title','ingredients','time','serving','recipe','nutrition','image']]

    # 네번째 데이터셋
    file_path = '../../2.데이터셋(프로젝트_클러스터링)/lovingitvegan_review_all.json'
    with open(file_path, 'r', encoding='utf-8-sig') as file:
        raw_data = json.load(file)

    df4=pd.DataFrame(raw_data["lovingitvegan"])
    df4=df4[['title','ingredients','time','serving','recipe','nutrition','image']]

    # 다섯번째 데이터셋
    file_path = '../../2.데이터셋(프로젝트_클러스터링)/minimalistbaker_review_all.json'
    with open(file_path, 'r', encoding='utf-8-sig') as file:
        raw_data = json.load(file)

    df5=pd.DataFrame(raw_data["minimalistbaker"])
    df5=df5[['title','ingredients','time','serving','recipe','nutrition','image']]

    # 여섯번째 데이터셋
    file_path = '../../2.데이터셋(프로젝트_클러스터링)/myplate_review_all.json'
    with open(file_path, 'r', encoding='utf-8-sig') as file:
        raw_data = json.load(file)

    df6=pd.DataFrame(raw_data["myplate"])
    df6=df6[['title','ingredients','time','serving','recipe','nutrition','image']]

    # 일곱번째 데이터셋
    file_path = '../../2.데이터셋(프로젝트_클러스터링)/pickuplimes_review_all.json'
    with open(file_path, 'r', encoding='utf-8-sig') as file:
        raw_data = json.load(file)

    df7=pd.DataFrame(raw_data["pickuplimes"])
    df7=df7[['title','ingredients','time','serving','recipe','nutrition','image']]

    # 여덟번째 데이터셋
    file_path = '../../2.데이터셋(프로젝트_클러스터링)/skinnytaste_crawling_all.json'
    with open(file_path, 'r', encoding='utf-8-sig') as file:
        raw_data = json.load(file)

    df8=pd.DataFrame(raw_data["skinnytaste"])
    df8=df8[['title','ingredients','time','serving','recipe','nutrition','image']]

    # 아홉번째 데이터셋
    file_path = '../../2.데이터셋(프로젝트_클러스터링)/thecuriouschickpea_review_all.json'
    with open(file_path, 'r', encoding='utf-8-sig') as file:
        raw_data = json.load(file)

    df9=pd.DataFrame(raw_data["thecuriouschickpea"])
    df9=df9[['title','ingredients','time','serving','recipe','nutrition','image']]


    #모든 데이터셋 합치기
    df= pd.concat([df1,df2,df3,df4,df5,df6,df7,df8,df9], ignore_index=True) #ignore_index를 사용하지 않으면 기존에 사용하는
...
def Clustering_and_save(selected_feature=50):
    df=C1_get_datasets()
    tokened_df,recipe_N_ingredients_2=C2_get_preprocessed_recipe(df)
    TF_IDF_matrix,tfidfv,vocabs= C3_TF_IDF(tokened_df,selected_feature)
    ...
    df.to_json('./Output/Preprocessed_recipe.json',orient='table')

    print('저장이 완료되었습니다')
    return df,clusters_df, vocabs

#%% 2 더미데이터 제작

#%% 2-1. 더미데이터 제작

def D1_make_dummy_simply():
    df= C1_get_datasets()
    ...
    dummy_df
    #확률을 긍정 리뷰인 확률을 2%로 설정
    random_numbers = [np.random.choice(np.arange(2),len(recipe_names), p=[0.98,0.02]) for i in range(dummy_user_len)]
...
#%% 3
# 콘텐츠 기반 필터링
# CBF 유저 데이터 전처리와 CF 유저 데이터 전처리를 통일시킴
#%% 3-1 &4-1. 유저 데이터 전처리하기
...
#%%
def CBF(User_ID,model_loc='Output/CBF_Model'):
    CBF_df= None
    selected_recipe_len=200
    try:
        #유저들의 평가 데이터 불러오기
...
def Make_CBF_model():
    ...
    #모델 저장하기
    fname = get_tmpfile("/Users/wooseongkyun/코드_아카이브/멀캠_프로젝트들/프로젝트3_머신러닝서비스/1.코드파일/프로젝트코드(for_Django)/Output/CBF_Model")
    model.save(fname)
...
#%% 4-R1 협업 필터링 적용
    #특정 유저의 좋아요 기록을 불러오기
def CF(user_id,model_loc="/Users/wooseongkyun/코드_아카이브/멀캠_프로젝트들/프로젝트3_머신러닝서비스/1.코드파일/프로젝트코드("
                         "for_Django)/Output/CF_Model.h5"
        ,top_n=10):
    ...
    #모델 불러오기
    model_loc= "/Users/wooseongkyun/코드_아카이브/멀캠_프로젝트들/프로젝트3_머신러닝서비스/1.코드파일/프로젝트코드(for_Django)/Output/CF_Model.h5"
    top_n=10
...
def CF_making_N_teaching(ratings,ratings_train,ratings_test):

    # Variable 초기화
    K = 200                             # Latent factor 수
    mu = ratings_train.like.mean()      # 전체 평균
    ...
    model.fit(
        ...
        y=ratings_train.like.values - mu,
        epochs=65,
        batch_size=512,
        validation_data=(
            [ratings_test.user_id.values, ratings_test.selected_recipe_id.values],
            ratings_test.like.values - mu
        )
    )
    #모델 저장
    filepath = "/Users/wooseongkyun/코드_아카이브/멀캠_프로젝트들/프로젝트3_머신러닝서비스/1.코드파일/프로젝트코드(for_Django)/Output/CF_Model.h5"
    model.save(filepath)

#%% 폐기 장소
# 혹 몰라 일단 여기 둠. 서버에 올릴땐 삭제해도 상관없을 듯

#%% 3-R1. CBF 추천 알고리즘 적용시키기
# def CBF(User_ID,model_loc='Output/CBF_Model'):
#     CBF_df= None
#     try:
#         User_lst= CBF_preprocessing()
#         # 모델 불러오기
#         model = doc2vec.Doc2Vec.load(model_loc)
#         #특정 유저의 좋아요 기록을 불러오기
#         user_log= User_lst[User_ID]
#         #임베딩 벡터 평균치로써 유저가 가장 좋아할만한 레시피 10개를 추천한다
#         recommend_result=model.dv.most_similar(user_log,topn=10)
#
#         #이때 데이터는 (레시피명,유사도) 튜플 형태로 반환된다
#         #추천된 레시피와 유사도 점수를 분리해서 담기
#         recipe_name=[recommend_result[i][0] for i in range(len(recommend_result))]
#         similarity_score=[recommend_result[i][1] for i in range(len(recommend_result))]
#         CBF_df= pd.DataFrame([recipe_name,similarity_score,User_lst[User_ID]],
#                              index=['recommended_recipe','ingredinets_cosine_similarity','user_preferred_recipe']).T
#
#         #ver2. 성능 비교를 위해 둠
#         # recipe_name=[]
...
#         #
#         # for i in range(len(recommend_result)):
#         #     recipe_name.append(recommend_result[i][0])
#         #     similarity_score.append(recommend_result[i][1])
#         #
#         # CBF_df= pd.DataFrame([recipe_name,similarity_score,User_lst[User_ID]],
#         #                       index=['recommended_recipe','ingredinets_cosine_similarity','user_preferred_recipe']).T
#
...
#%% 3-1.유저 데이터 전처리하기
# def CBF_preprocessing():
#
#     user_lst=None
#     try :
#         #경로 설정 / 'Unnamed: 0' 열 존재시 삭제
#         user_df= pd.read_csv('Output/Dummy_Data.csv')
#         try:
...
#
#         #유저 데이터셋의 레피시명을 추출한다
#         user_colname =list(user_df.columns)
#
#         #유저가 좋아요(1)를 누른 위치의 인덱스를 리스트에 담는다
#         #유저가 별점 4또는 5점이상을 준 평가를 리스트에 담는다
#         index_lst=[list((user_df.iloc[i]==4).values) for i in range(len(user_df))] + \
#                   [list((user_df.iloc[i]==5).values) for i in range(len(user_df))]
#
#         # for i in range(len(user_df)):
#         #     index_lst.append(list((user_df.iloc[i]==4).values))
#         #     index_lst.append(list((user_df.iloc[i]==5).values))
#
#         user_lst=[]
#         recipe_log=list((pd.Series(user_colname)[index_lst[i]]).values for i in range(len(user_df)))
#         ##유저 데이터를 [[유저1의 좋아요 음식명],[유저2의 좋아요 음식명],...,[유저n의 좋아요 음식명]]으로 적재한다
#         for i in range(len(user_df)):
#             recipe_log= list((pd.Series(user_colname)[index_lst[i]]).values)
#             user_lst.append(recipe_log)
#
#
#     except:
#         print('아직 유저 로그 데이터가 적재되어 있지 않습니다')
#     return user_lst
#

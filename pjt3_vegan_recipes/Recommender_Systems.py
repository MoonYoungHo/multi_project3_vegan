...
def C2_get_preprocessed_recipe(df):
        ...
        try:
            for j in range(len(recipe_N_ingredients.iloc[i]['ingredients'])):
                if type(recipe_N_ingredients.iloc[i]['ingredients'][j]) == str:
                    ingredients_lst[i] = ingredients_lst[i] + PreprocessText(
                    ...
                    ingredients_lst[i] = ingredients_lst[i] + \
                                         PreprocessText(str([x + '  ' for x in list(
    ...
    ingredient_stopwords = ['fresh', 'optional', 'sliced', 'cubes', 'hot', 'frozen', 'juiced', 'syrup', 'taste',
                            'unsweetened',
                            'soft', 'removed', 'plant', 'based', 'choice', 'tspground', 'turmeric', 'pinchground',
                            'black', 'canned', ''
        , 'granulated', 'vegan', 'pure', 'extract', 'brown', 'boilng', 'powder', 'syrupagave', 'crushed', 'whole',
                            'cloves', 'dairy', 'free', 'dark', 'drained', 'ground', 'medium', 'vegatable', 'bouillon',
                            'cooked',
                            'small', 'yellow', 'bell', 'sauce', 'nutritional', 'chopped', 'red', 'cut', 'thinly', 'dry',
                            'white',
                            'baking', 'minced', 'dried', 'peeled', 'purpose', 'roasted', 'oregano', 'rolled', 'diced',
                            'raw',
                            'extra', 'large', 'water', 'leaves', 'green', 'plus', 'juice', 'light', 'divided', 'melted',
                            'plain',
                            'rinsed', 'fat', 'seeds', 'toasted', 'clove', 'flakes', 'shredded', 'finely', 'grated',
                            'roughly',
                            'freshly', 'sweet', 'sea', 'packed', 'ripe', 'like', 'virgin', 'smoked', 'organic',
                            'bought', 'use',
                            'needed', 'serve', 'pinch', 'recipe', 'sub', 'gf', 'adjust', 'ounces', 'tablespoons',
                            'handful', 'used',
                            'teaspoons', 'chips', 'slices', 'pieces', 'less', 'soaked', 'half', 'pitted', 'low', 'thin',
                            'store',
                            'baby', 'see', 'kosher', 'non', 'fine', 'not', 'found', 'tablespoon', 'ice', 'cooking',
                            'full', 'firm',
                            'gluten', 'paste', 'garnish', 'bunch', 'yields', 'written', 'halved', 'stock', 'spice',
                            'mix', 'cayenne',
                            'spray', 'spring', 'heaped', 'vegetable', 'powdered', 'topping', 'mixed', 'caster']

    volume = ['ml', 'mL', 'cc', 'l', 'L', 'liter', 'dl', 'dL', 'teaspoon', 't', 'tsp', 'tablespoon', \
              'T', 'tbl', 'tbs', 'tsp', 'Tbsp', 'tbsp', 'fl oz', 'gill', 'cup', 'cups', 'c', 'pint', 'p', 'pt', 'fl pt',
              'quart', \
...
def Recommended_RecipeData_by_CBF(user_id):
    ...
    #Broken pipe from ('127.0.0.1', 56312)
    #Internal Server Error: /Recommend_by_algorithm/
    # Recommender_df= pd.read_json(BASE_DIR+'/Output/CBF_Recommender/'+'User_ID_'+str(user_id)+'_CBF_results.json')
